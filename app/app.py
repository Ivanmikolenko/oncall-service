...
# Режим Redis: "sentinel" или "single" (для локальной разработки)
REDIS_MODE          = os.getenv("REDIS_MODE", "single")
REDIS_HOST          = os.getenv("REDIS_HOST", "localhost")
REDIS_PORT          = int(os.getenv("REDIS_PORT", "6379"))
# Для Sentinel: "host1:port1,host2:port2,host3:port3"
...
# --------------------------------------------------------------------------- #
db_pool  = None
rds      = None   # объект для чтения (master или single)
rds_write = None  # объект для записи (всегда master)
...
def init_redis():
    global rds, rds_write
    if REDIS_MODE == "sentinel" and REDIS_SENTINEL_HOSTS:
        # Парсим "host1:port1,host2:port2,host3:port3"
        sentinels = []
        ...
        rds       = sentinel.slave_for(REDIS_SENTINEL_MASTER, decode_responses=True)
        rds_write = sentinel.master_for(REDIS_SENTINEL_MASTER, decode_responses=True)
        logger.info("redis mode=sentinel master=%s sentinels=%s", REDIS_SENTINEL_MASTER, sentinels)
    else:
        # Режим single — для локальной разработки без Sentinel
        client = redis.Redis(
            host=REDIS_HOST, port=REDIS_PORT,
            socket_connect_timeout=1, socket_timeout=1,
            decode_responses=True,
        )
        rds = rds_write = client
        logger.info("redis mode=single host=%s port=%s", REDIS_HOST, REDIS_PORT)
...
def get_note(note_id):
    ...
    title, body = row
    try:
        rds_write.setex(cache_key, REDIS_TTL, f"{title}\x1f{body}")
    except redis.RedisError as e:
...
def invalidate(note_id):
    try:
        rds_write.delete(f"note:{note_id}")
...
def metrics():
    ...
    for (method, path), total in _metrics["request_duration_sum"].items():
        lines.append(
            f'app_request_duration_seconds_sum{{method="{method}",path="{path}"}} {total}'
        )
    for (method, path), cnt in _metrics["request_duration_count"].items():
        lines.append(
            f'app_request_duration_seconds_count{{method="{method}",path="{path}"}} {cnt}'
        )
...
def bootstrap():
    ...
    init_redis()
    logger.info(
        "service bootstrapped: db=%s:%s redis_mode=%s", DB_HOST, DB_PORT, REDIS_MODE
    )
